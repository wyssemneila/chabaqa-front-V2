...
export type BlockDef = { id: string; type: BlockType; label: Localized; desc: Localized; tint: string; visible: boolean; accent?: string; font?: string; code?: string };
...
function darkenColor(hex: string, amount = 0.18): string {
  ...
  const channels = [1, 3, 5].map((offset) => Math.max(0, Math.round(parseInt(hex.slice(offset, offset + 2), 16) * (1 - amount))).toString(16).padStart(2, '0'));
...
export function Section({ block, c, design, device, index, media, activeMedia, setActiveMedia, reviews, page, setPage, t, openFaq, setOpenFaq, openSec, setOpenSec }: SectionProps) {
  ...
  const mobile = isMobile(device); const width = sectionWidth(device); const radius = design.radius; const accent = block.accent || design.accent; const accent2 = block.accent ? darkenColor(block.accent) : design.accent2;
  ...
  switch (block.type) {
    ...
    case 'about': return wrap(<div style={{ display: 'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 1fr', gap: 55, alignItems: 'center' }}><div><span style={{ color: accent, fontWeight: 800 }}>{c.aboutEyebrow}</span><h2 style={{ fontFamily: design.headingFont, fontSize: mobile ? 32 : 46, lineHeight: 1.12 }}>{c.aboutTitle}</h2><p style={{ color: '#74687e', fontSize: 18, lineHeight: 1.8 }}>{c.aboutDescription}</p></div><div style={{ padding: mobile ? 25 : 38, borderRadius: radius + 8, color: '#fff', background: `linear-gradient(145deg,${accent},${accent2})`, boxShadow: `0 22px 55px ${accent}35` }}><Rocket size={35} /><h3 style={{ fontFamily: design.headingFont, fontSize: 27 }}>{c.aboutCardTitle}</h3><p style={{ opacity: .88, lineHeight: 1.7 }}>{c.aboutCardDescription}</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginTop: 28 }}>{[[c.members,c.aboutMembersLabel],[c.online,c.aboutOnlineLabel],[c.admins,c.aboutHostsLabel]].map(([n,l]) => <div key={l}><strong style={{ display: 'block', fontSize: 23 }}>{n}</strong><small style={{ opacity: .75 }}>{l}</small></div>)}</div></div></div>);
