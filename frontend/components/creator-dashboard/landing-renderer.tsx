...
export type LandingContent = {
  ...
  curriculumTitle: string; curriculumDescription: string; curriculum: string[];
  testimonialsTitle: string; pricingTitle: string; pricingDescription: string;
...
export const DEFAULT_CONTENT: LandingContent = {
  ...
  creatorRole: 'Creator educator & community builder', creatorBio: 'Lina has helped thousands of independent creators turn their knowledge into meaningful products and thriving communities.',
  heroEyebrow: 'A community built for your next chapter', heroTitle: 'Turn your creative ambition into real momentum',
  ...
  aboutTitle: 'Build alongside people who understand', aboutDescription: 'No more figuring it all out alone. Learn proven systems, share your wins and challenges, and create work you are proud to put into the world.',
  curriculumTitle: 'A clear path from idea to impact', curriculumDescription: 'Short, practical modules that meet you where you are and help you take the next useful step.',
  ...
  pricingDescription: 'Get every lesson, resource, workshop, and community conversation in one simple membership.',
  ...
  ctaSecondary: 'Explore products', footerDescription: 'A thoughtful home for creators ready to learn, build, and grow together.',
};
  ...
  switch (block.type) {
    case 'hero': {
      ...
      return <section style={{ overflow: 'hidden', fontFamily: stack(block.font || design.bodyFont), background: design.bg === 'white' ? '#fff' : design.bg === 'tint' ? '#f8f4ff' : `radial-gradient(circle at 85% 15%, ${design.accent2}28, transparent 33%), linear-gradient(145deg,#fff 15%,#f6efff 100%)` }}><div style={{ width, margin: '0 auto', padding: mobile ? '64px 20px 55px' : '90px 0 78px', display: 'grid', gridTemplateColumns: mobile ? '1fr' : '1.04fr .96fr', gap: mobile ? 42 : 65, alignItems: 'center' }}>
        <div>{brandLockup}<span style={{ display: 'inline-flex', gap: 7, alignItems: 'center', color: accent, fontWeight: 800, background: `${accent}12`, padding: '8px 13px', borderRadius: 999, fontSize: 13 }}><Sparkles size={15} />{c.heroEyebrow}</span><h1 style={{ fontFamily: design.headingFont, fontSize: mobile ? 40 : device === 'tablet' ? 52 : 66, lineHeight: 1.02, letterSpacing: '-.045em', margin: '22px 0', color: '#25162f' }}>{c.heroTitle}</h1><p style={{ fontSize: mobile ? 17 : 19, lineHeight: 1.7, color: '#71647a', maxWidth: 610 }}>{c.heroDescription}</p><div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, margin: '30px 0 25px' }}><button style={buttonStyle(design)}>{c.ctaPrimary}<ArrowRight size={18} /></button>{design.showProducts && <button onClick={() => setPage('products')} style={buttonStyle(design, true)}><ShoppingBag size={18} />{c.ctaSecondary}</button>}</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, color: '#6f6478', fontSize: 14 }}><span><Users size={15} style={{ verticalAlign: -3 }} /> {c.members} members</span><span><Star size={15} fill="#f59e0b" color="#f59e0b" style={{ verticalAlign: -3 }} /> {c.rating} ({c.reviews})</span><span><BookOpen size={15} style={{ verticalAlign: -3 }} /> {c.lessons} lessons</span></div></div>
    ...
    case 'about': return wrap(<div style={{ display: 'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 1fr', gap: 55, alignItems: 'center' }}><div><span style={{ color: accent, fontWeight: 800 }}>WHY JOIN</span><h2 style={{ fontFamily: design.headingFont, fontSize: mobile ? 32 : 46, lineHeight: 1.12 }}>{c.aboutTitle}</h2><p style={{ color: '#74687e', fontSize: 18, lineHeight: 1.8 }}>{c.aboutDescription}</p></div><div style={{ padding: mobile ? 25 : 38, borderRadius: radius + 8, color: '#fff', background: `linear-gradient(145deg,${accent},${design.accent2})`, boxShadow: `0 22px 55px ${accent}35` }}><Rocket size={35} /><h3 style={{ fontFamily: design.headingFont, fontSize: 27 }}>Learn. Apply. Grow.</h3><p style={{ opacity: .88, lineHeight: 1.7 }}>A community experience built around meaningful action—not endless information.</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginTop: 28 }}>{[[c.members,'Members'],[c.online,'Online'],[c.admins,'Hosts']].map(([n,l]) => <div key={l}><strong style={{ display: 'block', fontSize: 23 }}>{n}</strong><small style={{ opacity: .75 }}>{l}</small></div>)}</div></div></div>);
    case 'curriculum': return wrap(<>{title(c.curriculumTitle, c.curriculumDescription)}<div style={{ maxWidth: 780, margin: '0 auto' }}>{c.curriculum.map((item, i) => { const opened = openSec === i; return <button type="button" key={item} onClick={() => setOpenSec(opened ? null : i)} style={{ width: '100%', textAlign: 'left', border: '1px solid #e9e2ef', borderRadius: radius, background: '#fff', marginBottom: 12, padding: 20, cursor: 'pointer', color: '#30243a' }}><span style={{ display: 'flex', alignItems: 'center', gap: 15 }}><span style={{ color: accent, fontWeight: 800, background: `${accent}12`, width: 36, height: 36, borderRadius: 12, display: 'grid', placeItems: 'center' }}>{i + 1}</span><strong style={{ flex: 1, fontSize: 17 }}>{item}</strong><ChevronDown size={19} style={{ transform: opened ? 'rotate(180deg)' : 'none', transition: '.2s' }} /></span>{opened && <span style={{ display: 'block', color: '#75697e', lineHeight: 1.65, padding: '14px 10px 0 51px' }}>Practical guidance, examples, and an action step to turn this lesson into progress.</span>}</button>; })}</div></>);
    case 'creator': return wrap(<div style={{ maxWidth: 850, margin: '0 auto', display: 'grid', gridTemplateColumns: mobile ? '1fr' : '220px 1fr', gap: 38, alignItems: 'center' }}><div style={{ aspectRatio: '1', borderRadius: radius + 12, background: `linear-gradient(145deg,${accent},${design.accent2})`, color: '#fff', display: 'grid', placeItems: 'center', font: `800 60px ${design.headingFont}` }}>{c.creatorName.split(' ').map((x) => x[0]).slice(0,2).join('')}</div><div><span style={{ color: accent, fontWeight: 800 }}>MEET YOUR HOST</span><h2 style={{ font: `800 ${mobile ? 30 : 40}px ${design.headingFont}`, margin: '10px 0 5px' }}>{c.creatorName}</h2><p style={{ color: accent, fontWeight: 700 }}>{c.creatorRole}</p><p style={{ color: '#74687e', lineHeight: 1.8, fontSize: 17 }}>{c.creatorBio}</p><div style={{ display: 'flex', gap: 10 }}>{[Instagram, Linkedin, Twitter].map((Icon, i) => <span key={i} style={{ width: 36, height: 36, borderRadius: 99, background: `${accent}12`, color: accent, display: 'grid', placeItems: 'center' }}><Icon size={17} /></span>)}</div></div></div>);
    ...
    case 'pricing': return wrap(<>{title(c.pricingTitle, c.pricingDescription)}<div style={{ maxWidth: 480, margin: '0 auto', padding: mobile ? 27 : 40, borderRadius: radius + 8, background: '#fff', border: `2px solid ${accent}35`, boxShadow: `0 24px 65px ${accent}18`, position: 'relative' }}><span style={{ position: 'absolute', top: -14, left: '50%', transform: 'translateX(-50%)', background: accent, color: '#fff', borderRadius: 99, padding: '7px 15px', fontWeight: 800, fontSize: 12 }}>MOST POPULAR</span><div style={{ textAlign: 'center', margin: '16px 0 25px' }}><span style={{ textDecoration: 'line-through', color: '#a49aa9' }}>{c.currency}{c.origPrice}</span><div><strong style={{ font: `800 54px ${design.headingFont}`, color: '#271a31' }}>{c.currency}{c.price}</strong> <span style={{ color: '#7a6f82' }}>/ {c.period}</span></div></div>{[c.access, `${c.lessons} practical lessons`, 'All workshops and recordings', 'Private community access', 'New resources every month'].map((x) => <div key={x} style={{ display: 'flex', gap: 10, padding: '10px 0', color: '#62566b' }}><Check size={19} color={accent} /><span>{x}</span></div>)}<button style={{ ...buttonStyle(design), width: '100%', justifyContent: 'center', marginTop: 23 }}>{c.ctaPrimary}<ArrowRight size={18} /></button><p style={{ textAlign: 'center', color: '#928798', fontSize: 12 }}><LockKeyhole size={12} style={{ verticalAlign: -2 }} /> Secure checkout · Instant access</p></div></>);
