...
export function useWhatsappPage(communityId: string | null) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const [campaignRes, contactRes, statsRes, automationRes] = await Promise.all([
  ...
  const importContact = useCallback(async (name: string, phoneE164: string) => {
    if (!communityId) throw new Error("Select a community first")
    await whatsappApi.importContacts(communityId, [{ name, phoneE164, optIn: true }])
    await load()
  }, [communityId, load])

  const previewAudience = useCallback(async (targetAudience: WhatsappAudienceType = "all_members") => {
  ...
  const toggleAutomation = useCallback(async (automation: WhatsappAutomation) => {
    if (!communityId) throw new Error("Select a community first")
    await whatsappApi.updateAutomation(communityId, automation._id, { isActive: !automation.isActive })
    await load()
  }, [communityId, load])
