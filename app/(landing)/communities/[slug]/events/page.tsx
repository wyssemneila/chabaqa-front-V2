...
import { Calendar, Ticket, Video, MapPin, Clock } from 'lucide-react'
      ...
      {community.events.length === 0 ? (
        <div className="rounded-2xl p-12 text-center" style={{ background: 'var(--white)', border: '1px solid var(--bd)' }}>
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-3" style={{ background: 'var(--p2)' }}>
            <Calendar className="w-7 h-7" style={{ color: 'var(--p)' }} strokeWidth={1.3} />
          </div>
              ...
              <article key={event.id}
                ...
                <div className="p-5 flex items-start gap-5">
                  ...
                  <div className="rounded-2xl overflow-hidden flex-shrink-0 text-center" style={{ width: 60, border: '1px solid var(--bd)' }}>
                    <div className="py-1.5 text-[11px] font-extrabold text-white" style={{ background: 'var(--p)' }}>
                      {month?.toUpperCase()}
                    </div>
                    <div className="py-2 px-1">
                      <p className="text-2xl font-extrabold leading-none" style={{ color: 'var(--t1)' }}>{day?.replace(',', '')}</p>
                      <p className="text-[11px] mt-0.5" style={{ color: 'var(--t3)' }}>{year}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      {soldOut && (
                        <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full" style={{ background: '#fee2e2', color: '#ef4444' }}>
                          {isAr ? 'نفدت التذاكر' : 'Sold Out'}
                        </span>
                      )}
                      {event.registered && (
                        <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full" style={{ background: '#d1fae5', color: '#10b981' }}>
                  ...
                  <div className="flex-shrink-0 flex flex-col items-end gap-3 self-start">
                    <span className="text-base font-extrabold" style={{ color: isFree ? '#10b981' : 'var(--p)' }}>
                      {isFree ? (isAr ? 'مجاني' : 'Free') : `${event.price} ${event.currency ?? ''}`}
                    </span>
                    <button
                      disabled={soldOut && !event.registered}
                      className="px-4 py-2.5 rounded-xl text-sm font-semibold transition-all hover:opacity-90 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
