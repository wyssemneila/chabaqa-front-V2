...
export default async function ChallengesPage({ params }: Props) {
  ...
  const challenges = [...community.challenges].sort((a, b) => order[a.status] - order[b.status])
        ...
        {challenges.map((challenge) => {
          ...
          return (
            <CardWrap key={challenge.id} {...wrapProps}
              ...
              {/* Cover 16:9 — banner image if available */}
              <div className="relative aspect-[16/9] w-full overflow-hidden bg-gray-100">
                {challenge.banner ? (
                  /* eslint-disable-next-line @next/next/no-img-element */
                  <img src={challenge.banner} alt={challenge.title}
                       className={`w-full h-full object-cover ${locked ? '' : 'transition-transform group-hover:scale-105'}`} />
                ) : (
                  <div className="w-full h-full"
                       style={{ background: 'linear-gradient(135deg, #8e78fb 0%, #a08cff 55%, #c4b8fd 130%)' }} />
                )}
              ...
              <div className="p-3.5 flex-1 flex flex-col gap-2">
                ...
                {isAdmin ? (
                  ...
                  <div className="w-full mt-2 py-2 text-[12px] font-semibold rounded-lg text-white text-center flex items-center justify-center gap-1.5 transition-opacity group-hover:opacity-90"
                       style={{ background: challenge.status === 'active' ? '#8e78fb' : '#ff9b28' }}>
